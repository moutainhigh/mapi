...
class SelfSignController extends BaseController {
    ...
    def confirm = {
        try {
            ...
            Matcher matcher = pattern.matcher(params.amount);
            if (!matcher.find()) {
                flash.errorMessage = "单笔支付限额书写错误!"
                render view: 'index'
                return
            }
            if (params.amount != '' && Double.parseDouble(params.amount) <= 0) {
                flash.errorMessage = "单笔支付限额应大于零!"
                render view: 'index'
                return
            }
            if (params.amount != '' && Double.parseDouble(params.amount) > 100000000) {
                flash.errorMessage = "单笔支付限额最大为100000000元!"
                render view: 'index'
                return
            }
            binding = new BindingMoney(
                    partner: partner,
                    customer: slave,
                    nopassRefundFlag: 'T',
                    ...
                    amount: params.amount != '' ? params.amount : '50000',
                    totalAmount: '50000'
            )
